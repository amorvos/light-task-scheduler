...
public abstract class AbstractRemoting {
	...
	protected final ConcurrentHashMap<Integer /* opaque */, ResponseFuture> responseTable = new ConcurrentHashMap<Integer, ResponseFuture>(
			256);
	// 注册的各个RPC处理器
	protected final HashMap<Integer/* request code */, Pair<RemotingProcessor, ExecutorService>> processorTable = new HashMap<Integer, Pair<RemotingProcessor, ExecutorService>>(
			64);
	...
	public void processRequestCommand(final Channel channel, final RemotingCommand cmd) {
		...
		if (pair != null) {
				...
				try {
						...
						if (response != null) {
							...
							try {
								channel.writeAndFlush(response).addListener(future -> {
									if (!future.isSuccess()) {
										LOGGER.error("response to " + RemotingHelper.parseChannelRemoteAddr(channel)
												+ " failed", future.cause());
					...
					if (!RemotingCommandHelper.isOnewayRPC(cmd)) {
						final RemotingCommand response = RemotingCommand.createResponseCommand(
								RemotingProtos.ResponseCode.SYSTEM_ERROR.code(), //
			...
			try {
				...
				if (!RemotingCommandHelper.isOnewayRPC(cmd)) {
					final RemotingCommand response = RemotingCommand.createResponseCommand(
							RemotingProtos.ResponseCode.SYSTEM_BUSY.code(),
			...
			final RemotingCommand response = RemotingCommand
					.createResponseCommand(RemotingProtos.ResponseCode.REQUEST_CODE_NOT_SUPPORTED.code(), error);
			response.setOpaque(cmd.getOpaque());
			channel.writeAndFlush(response);
	...
	public void processResponseCommand(Channel channel, RemotingCommand cmd) {
		...
		if (responseFuture != null) {
			...
			LOGGER.warn(
					"receive response, but not matched any request, " + RemotingHelper.parseChannelRemoteAddr(channel));
	...
	public void invokeAsyncImpl(final Channel channel, final RemotingCommand request, final long timeoutMillis,
		...
		if (acquired) {
			...
			final ResponseFuture responseFuture = new ResponseFuture(request.getOpaque(), timeoutMillis, asyncCallback,
					once);
			this.responseTable.put(request.getOpaque(), responseFuture);
			try {
				channel.writeAndFlush(request).addListener(future -> {
			...
			if (timeoutMillis <= 0) {
				...
				LOGGER.warn("invokeAsyncImpl tryAcquire semaphore timeout, " + timeoutMillis + " waiting thread nums: "
	...
	public void invokeOnewayImpl(final Channel channel, final RemotingCommand request, final long timeoutMillis)
		...
		if (acquired) {
			...
			if (timeoutMillis <= 0) {
				...
				LOGGER.warn("invokeOnewayImpl tryAcquire semaphore timeout, " + timeoutMillis + " waiting thread nums: "
