...
public class MStatReportWorker implements Runnable {
	...
	private PriorityBlockingQueue<MData> queue = new PriorityBlockingQueue<>(16,
			Comparator.comparing(MData::getTimestamp));
	...
	public MStatReportWorker(AppContext appContext, AbstractMStatReporter reporter) {
		...
		this.loadBalance = ServiceLoader.load(LoadBalance.class, appContext.getConfig(),
				ExtConfig.MONITOR_SELECT_LOADBALANCE);
	}
	...
	public void run() {
		...
		synchronized (MStatReportWorker.class) {
			if (running) {
				return;
			}
			running = true;
