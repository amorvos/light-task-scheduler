...
public abstract class AbstractRegistry implements Registry {
	...
	private final Set<Node> registered = Sets.newConcurrentHashSet();
	...
	public void subscribe(Node node, NotifyListener listener) {
		...
		if (listeners == null) {
			subscribed.putIfAbsent(node, Sets.newConcurrentHashSet());
		...
		if (!destroyRegistered.isEmpty()) {
			for (Node node : new HashSet<Node>(getRegistered())) {
				try {
					...
					LOGGER.warn("Failed to unregister node " + node + " to registry " + getNode()
			...
			for (Map.Entry<Node, Set<NotifyListener>> entry : destroySubscribed.entrySet()) {
				...
				for (NotifyListener listener : entry.getValue()) {
					try {
						...
						LOGGER.warn("Failed to unsubscribe node " + node + " to registry " + getNode()
								+ " on destroy, cause: " + t.getMessage(), t);
					}
				}
			}
