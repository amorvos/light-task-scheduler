...
public class HeartBeatMonitor {
	...
	private final ScheduledExecutorService PING_EXECUTOR_SERVICE = Executors.newScheduledThreadPool(1,
			new NamedThreadFactory("LTS-HeartBeat-Ping", true));
	...
	private final ScheduledExecutorService FAST_PING_EXECUTOR = Executors.newScheduledThreadPool(1,
			new NamedThreadFactory("LTS-HeartBeat-Fast-Ping", true));
	...
	public HeartBeatMonitor(RemotingClientDelegate remotingClient, AppContext appContext) {
		...
		this.jobTrackerUnavailableEventSubscriber = new EventSubscriber(
				HeartBeatMonitor.class.getName() + "_PING_" + appContext.getConfig().getIdentity(), eventInfo -> {
					startFastPing();
					stopPing();
				});
		appContext.getEventCenter().subscribe(new EventSubscriber(
				HeartBeatMonitor.class.getName() + "_NODE_ADD_" + appContext.getConfig().getIdentity(), eventInfo -> {
	...
	private void startPing() {
		try {
			if (pingStart.compareAndSet(false, true)) {
				...
				appContext.getEventCenter().subscribe(jobTrackerUnavailableEventSubscriber,
						EcTopic.NO_JOB_TRACKER_AVAILABLE);
	...
	private void stopPing() {
		try {
			if (pingStart.compareAndSet(true, false)) {
				appContext.getEventCenter().unSubscribe(EcTopic.NO_JOB_TRACKER_AVAILABLE,
						jobTrackerUnavailableEventSubscriber);
	...
	private boolean beat(RemotingClientDelegate remotingClient, String addr) {
		...
		RemotingCommand request = RemotingCommand.createRequestCommand(JobProtos.RequestCode.HEART_BEAT.code(),
				commandBody);
		try {
			RemotingCommand response = remotingClient.getRemotingClient().invokeSync(addr, request, 5000);
			if (response != null && JobProtos.ResponseCode.HEART_BEAT_SUCCESS == JobProtos.ResponseCode
