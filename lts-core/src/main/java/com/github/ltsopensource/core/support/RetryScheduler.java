...
public abstract class RetryScheduler<T> {
	...
	public RetryScheduler(String name, final AppContext appContext, String storePath) {
		...
		EventSubscriber subscriber = new EventSubscriber(
				RetryScheduler.class.getSimpleName().concat(appContext.getConfig().getIdentity()), eventInfo -> {
	...
	public void start() {
		try {
			if (selfCheckStart.compareAndSet(false, true)) {
				this.RETRY_EXECUTOR_SERVICE = Executors
						.newSingleThreadScheduledExecutor(new NamedThreadFactory("LTS-RetryScheduler-retry", true));
				// 这个时间后面再去优化
	...
	private void startMasterCheck() {
		try {
			if (masterCheckStart.compareAndSet(false, true)) {
				this.MASTER_RETRY_EXECUTOR_SERVICE = Executors.newSingleThreadScheduledExecutor(
						new NamedThreadFactory("LTS-RetryScheduler-master-retry", true));
				// 这个时间后面再去优化
				...
				LOGGER.info("Start {} master RetryScheduler success, identity=[{}]", name,
						appContext.getConfig().getIdentity());
			}
		} catch (Throwable t) {
			LOGGER.error("Start {} master RetryScheduler failed, identity=[{}]", name,
					appContext.getConfig().getIdentity(), t);
		}
	}
	...
	private void stopMasterCheck() {
		try {
			if (masterCheckStart.compareAndSet(true, false)) {
				...
				LOGGER.info("Stop {} master RetryScheduler success, identity=[{}]", name,
						appContext.getConfig().getIdentity());
			}
		} catch (Throwable t) {
			LOGGER.error("Stop {} master RetryScheduler failed, identity=[{}]", name,
					appContext.getConfig().getIdentity(), t);
		}
	}
			...
			try {
				...
				for (FailStore store : failStores) {
					...
					while (true) {
						...
						if (CollectionUtils.isEmpty(pairs)) {
							store.destroy();
							LOGGER.info("{} RetryScheduler, delete store dir[{}] success, identity=[{}] ", name,
									store.getPath(), appContext.getConfig().getIdentity());
							break;
						}
						List<T> values = new ArrayList<T>(pairs.size());
						List<String> keys = new ArrayList<String>(pairs.size());
				...
				LOGGER.error("Run {} master RetryScheduler error, identity=[{}] ", name,
						appContext.getConfig().getIdentity(), e);
			} finally {
