...
public class JobUtils {
	...
	static {
		Map<String, PropConverter<?, ?>> jobPoConverterMap = Maps.newConcurrentMap();
		// 目前只有这个 extParams和 internalExtParams 不是基本类型, 为了不采用流的方式, 从而提升性能
		jobPoConverterMap.put("extParams",
				(PropConverter<JobPo, Map<String, String>>) jobPo -> BeanUtils.copyMap(jobPo.getExtParams()));
		jobPoConverterMap.put("internalExtParams",
				(PropConverter<JobPo, Map<String, String>>) jobPo -> BeanUtils.copyMap(jobPo.getInternalExtParams()));
		JOB_PO_BEAN_COPIER = BeanCopierFactory.createCopier(JobPo.class, JobPo.class, jobPoConverterMap);

		Map<String, PropConverter<?, ?>> jobConverterMap = Maps.newConcurrentMap();
		// 目前只有这个 extParams不是基本类型, 为了不采用流的方式, 从而提升性能
		jobConverterMap.put("extParams",
				(PropConverter<Job, Map<String, String>>) job -> BeanUtils.copyMap(job.getExtParams()));
