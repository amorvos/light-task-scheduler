	...
	public void run() {
		...
		try {
			...
			while (jobMeta != null) {
				...
				BizLoggerAdapter logger = (BizLoggerAdapter) BizLoggerFactory.getLogger(appContext.getBizLogLevel(),
						appContext.getRemotingClient(), appContext);

				try {
					appContext.getRunnerPool().getRunningJobManager().in(jobMeta.getJobId(), this);
					...
					appContext.getRunnerPool().getRunningJobManager().out(jobMeta.getJobId());
				}
				// 统计数据
