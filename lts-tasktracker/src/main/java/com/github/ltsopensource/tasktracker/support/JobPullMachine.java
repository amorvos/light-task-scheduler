...
public class JobPullMachine {
	...
	public JobPullMachine(final TaskTrackerAppContext appContext) {
		...
		this.jobPullFrequency = appContext.getConfig().getParameter(ExtConfig.JOB_PULL_FREQUENCY,
				Constants.DEFAULT_JOB_PULL_FREQUENCY);
		...
		appContext.getEventCenter().subscribe(new EventSubscriber(
				JobPullMachine.class.getSimpleName().concat(appContext.getConfig().getIdentity()), new Observer() {
					@Override
		...
		try {
			if (start.compareAndSet(false, true)) {
				if (scheduledFuture == null) {
					scheduledFuture = executorService.scheduleWithFixedDelay(worker, jobPullFrequency * 1000,
							jobPullFrequency * 1000, TimeUnit.MILLISECONDS);
				}
	...
	private void sendRequest() throws RemotingCommandFieldCheckException {
		...
		RemotingCommand request = RemotingCommand.createRequestCommand(JobProtos.RequestCode.JOB_PULL.code(),
				requestBody);
