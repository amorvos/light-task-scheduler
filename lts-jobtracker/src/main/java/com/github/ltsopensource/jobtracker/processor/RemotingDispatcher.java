...
public class RemotingDispatcher extends AbstractRemotingProcessor {
	...
	private final Map<RequestCode, RemotingProcessor> processors = Maps.newHashMap();
	...
	public RemotingDispatcher(JobTrackerAppContext appContext) {
		...
		this.reqLimitEnable = appContext.getConfig().getParameter(ExtConfig.JOB_TRACKER_REMOTING_REQ_LIMIT_ENABLE,
				false);
		...
		this.reqLimitAcquireTimeout = appContext.getConfig()
				.getParameter(ExtConfig.JOB_TRACKER_REMOTING_REQ_LIMIT_ACQUIRE_TIMEOUT, 50);
	}
	 ...
	 * 限流处理
	 */
	private RemotingCommand doBizWithReqLimit(Channel channel, RemotingCommand request)
